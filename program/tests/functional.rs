...
async fn test_token_vesting() {
    ...
    let change_destination_instructions = [change_destination(
        &program_id,
        &vesting_account_key,
        &destination_account.pubkey(),
        &destination_token_account.pubkey(),
    ...
    let mut change_destination_transaction =
        Transaction::new_with_payer(&change_destination_instructions, Some(&payer.pubkey()));

    change_destination_transaction.partial_sign(&[&payer, &destination_account], recent_blockhash);

    banks_client
        .process_transaction(change_destination_transaction)
        .await
        .unwrap();
}
